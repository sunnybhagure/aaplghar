...
import { 
  ...
  IndianRupee, HousePlus, ChevronLeft, ChevronRight 
} from "lucide-react";

// --- 1. HORIZONTAL SLIDER WRAPPER ---
const PropertySlider = ({ children }) => {
  ...
  return (
    <div className="relative group px-2">
      ...
      <div ref={scrollRef} className="flex gap-6 overflow-x-auto no-scrollbar scroll-smooth pb-10 px-2" style={{ scrollSnapType: "x mandatory" }}>
        {children}
      </div>
...
// --- 2. PROPERTY CARD (Compact for Slider) ---
const PropertyCard = ({ property }) => {
  ...
  return (
    <div onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} className="min-w-[300px] md:min-w-[380px] bg-white rounded-[2rem] shadow-lg border border-slate-100 overflow-hidden transition-all hover:shadow-2xl scroll-snap-align-start group">
      <div className="h-56 overflow-hidden relative">
        <img src={currentImg} alt={property.title} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" />
        <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-md text-blue-700 text-[10px] font-black px-3 py-1.5 rounded-xl uppercase">
          {property.residentialDetails?.status?.replace("_", " ") || "New Launch"}
        </div>
      </div>
      <div className="p-6">
        <div className="flex justify-between items-start mb-2">
          ...
          <span className="text-emerald-600 font-black flex items-center text-sm">
            <IndianRupee className="w-3.5 h-3.5" /> {property.price?.starting}L - {property.price?.upto}L
          </span>
        </div>
        <div className="flex items-center gap-1 text-slate-500 mb-5">
          <MapPin className="w-4 h-4 text-rose-500" />
          <span className="text-xs font-bold">{property.location?.area}, {property.location?.city}</span>
        </div>
...
export default function HomePage() {
  ...
  const dummyData = Array(6).fill({
    title: "Aaple Ghar Premium",
    location: { city: "Pune", area: "Baner" },
    price: { starting: 45, upto: 85 },
    propertyType: "residential",
    images: {
        coverImage: "https://images.unsplash.com/photo-1567496898669-ee935f5f647a?w=800",
        gallery: ["https://images.unsplash.com/photo-1460317442991-0ec209397118?w=800"]
    },
    residentialDetails: { status: "under_construction", bhkTypes: [{ type: "2BHK" }] }
  });
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc]">
      ...
      {/* Hero Section - Exactly like your preferred code */}
          ...
          <div className="bg-white p-5 rounded-b-3xl rounded-tr-3xl shadow-2xl flex flex-col md:flex-row gap-4 items-center relative z-10">
            <div className="flex-1 w-full relative">
              ...
              <input 
                ...
                className="w-full pl-12 pr-4 py-4 rounded-2xl bg-slate-50 border-none focus:ring-2 focus:ring-blue-500 font-semibold text-slate-700"
              />
            </div>
      ...
      {/* Main Content Sections - Using Horizontal Sliders */}
      <div className="max-w-7xl mx-auto px-4 -mt-16 pb-32 relative z-20">
        ...
        <section className="mb-20">
          <div className="flex items-end justify-between mb-8 px-4">
            <div>
                <h2 className="text-3xl font-black text-slate-800 tracking-tight underline decoration-blue-500 decoration-4">Upcoming Projects</h2>
...
// Reusable Filter - Exactly Same
function FilterSelect({ label, options }) {
  return (
    <div className="flex flex-col gap-1.5">
      <label className="text-[10px] font-black text-slate-400 uppercase tracking-[2px] ml-1">{label}</label>
      <select className="bg-slate-50 border-none rounded-xl py-2.5 px-3 text-sm font-bold text-slate-700 focus:ring-2 focus:ring-blue-500 cursor-pointer">
