...
const BuilderDetails = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-20 overflow-x-hidden">
      {/* HEADER SECTION */}
      <div className="bg-white border-b border-slate-200 pt-6 md:pt-10 pb-6 md:pb-10 px-4 md:px-6">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-6 md:gap-10 items-center md:items-start">
          <div className="w-full max-w-[300px] md:w-80 h-48 md:h-60 bg-slate-50 rounded-3xl overflow-hidden border p-4 flex items-center justify-center shadow-sm">
            <img src={builder.coverImage} className="max-w-full max-h-full object-contain" alt="builder" />
          </div>
          <div className="flex-1 space-y-4 w-full">
            <div className="flex flex-wrap items-center gap-3">
              <h1 className="text-2xl md:text-4xl font-black text-slate-900 uppercase tracking-tighter italic break-words">{builder.companyName}</h1>
              <span className="bg-blue-600 text-white text-[9px] md:text-[10px] px-3 py-1 rounded-full font-black uppercase whitespace-nowrap">Verified</span>
            </div>
            
            <div className="flex flex-wrap gap-4 md:gap-6 text-slate-500 font-bold text-[10px] md:text-xs uppercase tracking-widest items-center">
              <div className="flex items-center gap-2"><Briefcase size={14} className="text-blue-500"/> {new Date().getFullYear() - builder.since} Yrs Exp.</div>
              <div className="flex items-center gap-2"><Building2 size={14} className="text-blue-500"/> {stats.totalProjects} Projects</div>
              <BuilderAverageRating builderId={id} />
              <div className="flex items-start gap-2 w-full md:w-auto md:border-l md:pl-6">
                <MapPin size={14} className="text-blue-500 shrink-0 mt-0.5"/> 
                <span className="text-slate-500 break-words leading-tight uppercase">
                  <span className="text-slate-400">Office:</span> {builder.companyAddress || "Not Available"}
                </span>
              </div>
            </div>

            <div className="relative">
              <p className={`text-xs md:text-sm text-slate-600 leading-relaxed ${!showFullAbout && 'line-clamp-3'}`}>{builder.about}</p>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
               <button className="bg-slate-900 text-white px-8 py-3 rounded-xl font-black text-[10px] md:text-xs uppercase tracking-widest flex items-center justify-center gap-3 shadow-xl"><Phone size={16}/> Contact Builder</button>
               <div className="flex items-start gap-2 text-slate-500 font-black text-[9px] md:text-[10px] uppercase tracking-widest bg-slate-50 px-4 py-3 rounded-xl border border-slate-100 overflow-hidden">
                  <Mail size={14} className="text-blue-500 shrink-0" /> 
                  <span className="break-all"><span className="text-slate-400 mr-1">Mail:</span> {builder.email || "N/A"}</span>
               </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 mt-6 md:mt-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        
        {/* RIGHT SIDE (Now comes FIRST on Mobile) */}
        <div className="lg:col-span-2 order-1 lg:order-2">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-full">
            <div className="p-4 md:p-6 border-b border-slate-100 space-y-4 md:space-y-6">
                <div className="flex gap-1 overflow-x-auto no-scrollbar pb-1">
                    {["residential", "commercial", "plot"].map((tab) => (
                        <button key={tab} onClick={() => { setActiveTab(tab); setSelectedType("All Types"); }} className={`flex items-center gap-2 px-5 py-3 rounded-xl font-black text-[9px] uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === tab ? "bg-blue-600 text-white shadow-lg" : "bg-slate-50 text-slate-400 hover:bg-slate-100"}`}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
                    <div className="relative" ref={cityMenuRef}>
                        <div onClick={() => setShowCityMenu(!showCityMenu)} className="bg-slate-50 rounded-xl py-2.5 px-3 md:px-4 text-[10px] md:text-[11px] font-bold text-slate-700 flex justify-between items-center cursor-pointer border border-slate-50">
            ...
            <div className={`divide-y divide-slate-50 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent ${filteredProperties.length > 12 ? 'max-h-[1100px]' : ''}`}>
              {filteredProperties.length > 0 ? filteredProperties.map((p) => {
                ...
                return (
                  <div key={p._id} onClick={() => navigate(`/property/${p._id}`)} className="flex flex-col sm:grid sm:grid-cols-12 gap-3 sm:items-center p-4 md:p-5 hover:bg-blue-50/40 transition-all cursor-pointer group">
                    <div className="sm:col-span-5 flex items-center gap-3 md:gap-4">
                      <img src={p.images?.coverImage} className="w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl object-cover border shadow-sm shrink-0" alt="p" />
                      <div className="overflow-hidden">
                        ...
                        <p className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1 truncate"><MapPin size={10}/> {p.location?.area}, {p.location?.city}</p>
                      </div>
                    </div>
                    <div className="flex justify-between items-center sm:block sm:col-span-3 px-0 sm:px-2">
                      <p className="text-[9px] md:text-[10px] font-black text-blue-600 bg-blue-50 px-2 py-1 rounded-md border border-blue-100 inline-block uppercase whitespace-nowrap">{getDisplaySpecs(p)}</p>
                    ...
                    <div className="sm:col-span-2 sm:text-right flex justify-between sm:flex-col sm:justify-center items-end sm:items-end">
        ...
        {/* LEFT SIDEBAR (Now comes SECOND on Mobile) */}
        <div className="lg:col-span-1 space-y-6 order-2 lg:order-1">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4">
            <div onClick={() => setSelectedStatus(selectedStatus === "ready" ? "All Status" : "ready")} className={`p-5 md:p-6 rounded-3xl flex justify-between items-center cursor-pointer border transition-all ${selectedStatus === "ready" ? "bg-green-600 border-green-700 shadow-xl" : "bg-green-50 border-green-100"}`}>
                <div><p className={`text-xl md:text-2xl font-black ${selectedStatus === "ready" ? "text-white" : "text-green-700"}`}>{stats.readyPossession}</p><p className={`text-[9px] md:text-[10px] font-black uppercase tracking-widest ${selectedStatus === "ready" ? "text-green-100" : "text-green-600"}`}>Ready Possession</p></div>
                <div className="w-10 h-10 md:w-12 md:h-12 bg-white rounded-2xl flex items-center justify-center text-green-600 shadow-sm shrink-0"><Sparkles size={18}/></div>
            </div>
            <div onClick={() => setSelectedStatus(selectedStatus === "under_construction" ? "All Status" : "under_construction")} className={`p-5 md:p-6 rounded-3xl flex justify-between items-center cursor-pointer border transition-all ${selectedStatus === "under_construction" ? "bg-orange-600 border-orange-700 shadow-xl" : "bg-orange-50 border-orange-100"}`}>
                <div><p className={`text-xl md:text-2xl font-black ${selectedStatus === "under_construction" ? "text-white" : "text-orange-700"}`}>{stats.underConstruction}</p><p className={`text-[9px] md:text-[10px] font-black uppercase tracking-widest ${selectedStatus === "under_construction" ? "text-orange-100" : "text-orange-600"}`}>Under Construction</p></div>
                <div className="w-10 h-10 md:w-12 md:h-12 bg-white rounded-2xl flex items-center justify-center text-orange-600 shadow-sm shrink-0"><Building2 size={18}/></div>
            </div>
          </div>

          <div className="bg-white p-5 md:p-6 rounded-3xl border border-slate-200 shadow-sm">
            <h4 className="text-[10px] md:text-[11px] font-black text-slate-400 uppercase tracking-[2px] mb-4">Work in Cities</h4>
            <div className="grid grid-cols-3 gap-2">
              {cities.map((city, idx) => (
                <div key={idx} className="bg-slate-50 border border-slate-100 p-2 rounded-lg text-center font-bold text-[9px] md:text-[10px] text-slate-600 uppercase truncate">{city}</div>
              ))}
            </div>
          </div>
...
const BuilderReviewSection = ({ builderId, currentUser }) => {
  ...
  return (
    <div className="bg-white p-5 md:p-6 rounded-3xl border border-slate-200 shadow-sm">
      <h4 className="text-[10px] md:text-[11px] font-black text-slate-400 uppercase tracking-[2px] mb-4">Reviews & Rating</h4>
          ...
          <div key={rev._id} className="border-b border-slate-50 pb-4 last:border-0">
            <div className="flex justify-between items-start">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 bg-blue-100 rounded-full flex items-center justify-center text-[10px] font-black text-blue-600 uppercase shrink-0">{rev.user?.name ? rev.user.name[0] : "?"}</div>
                <div>
                  <p className="text-[10px] font-black text-slate-800 uppercase truncate max-w-[120px]">{rev.user?.name || "User"}</p>
              ...
              {isOwner(rev) && (
                <button onClick={() => deleteReview(rev._id)} className="text-red-500 hover:text-red-700 shrink-0">
                  <Trash2 size={14} />
                </button>
              )}
            </div>
            <p className="text-[10px] font-bold text-slate-500 mt-2 leading-relaxed italic break-words">"{rev.comment}"</p>
          </div>
        ))}
      </div>
    </div>
...
function FilterSelect({ label, options, value, onChange }) {
  return (
    <select value={value} onChange={(e) => onChange(e.target.value)} className="w-full bg-slate-50 border-none rounded-xl py-2.5 px-3 md:px-4 text-[10px] md:text-[11px] font-bold text-slate-700 outline-none cursor-pointer">
